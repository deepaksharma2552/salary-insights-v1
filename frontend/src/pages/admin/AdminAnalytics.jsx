...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    const { from, to } = dateRange(RANGES[rangeIdx].days);
    setLoading(true);
    Promise.all([
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ background: 'var(--panel)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
            ...
            {dailyStats.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '24px 0', color: 'var(--text-3)', fontSize: 13, fontStyle: 'italic' }}>No data yet</div>
            ) : (
                ...
                {dailyStats.map((d, i) => {
                  ...
                  return (
                    <div key={i} title={`${d.date}: ${d.views} views`}
                      style={{ flex: '1 0 8px', maxWidth: 20, minWidth: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
